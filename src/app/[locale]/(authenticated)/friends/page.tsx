...
export default async function FriendsPage({ params }: { params: Promise<{ locale: string }> }) {
    ...
    if ('error' in dashboardData && dashboardData.error === 'Not authenticated') {
    ...
    return <FriendsClient initialData={combinedData} profile={('error' in dashboardData ? undefined : dashboardData.profile) as any} />;
}
